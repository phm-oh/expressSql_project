...
app.get('/', (req, res) => {
  res.json({
    message: '🔐 ยินดีต้อนรับสู่ User Authentication API!',
    ...
    endpoints: {
      ...
      'GET /api/users': 'ดึงข้อมูล user ทั้งหมด',
...
app.get('/test-db', async (req, res) => {
  try {
    ...
    res.json({
      success: isConnected,
      message: isConnected ? 'เชื่อมต่อ database สำเร็จ' : 'เชื่อมต่อ database ไม่สำเร็จ',
      timestamp: new Date().toISOString()
    });
  } catch (error) {
    res.status(500).json({
...
async function startServer() {
  try {
    console.log('🚀 กำลังเริ่มต้น User Authentication API Server...');
    console.log('=====================================');
    ...
    if (!isConnected) {
      ...
      console.error('   กรุณาแก้ไขการตั้งค่า database');
    }
    ...
    app.listen(PORT, () => {
      ...
      console.log(`🧪 ทดสอบ database: http://localhost:${PORT}/test-db`);
      console.log('=====================================');
      ...
      if (isConnected) {
        console.log('✅ พร้อมใช้งาน - database เชื่อมต่อสำเร็จ!');
      } else {
        console.log('⚠️  Server รัน แต่ database ไม่พร้อม!');
      }
    });
